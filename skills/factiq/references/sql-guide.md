...
30 seconds, and result sets are bounded — aggregate in SQL to the granularity
you actually need rather than pulling raw rows (see "Pivoting" and "Efficiency"
below). If the tool exposes `full` / `max_rows`, use it deliberately and keep
the returned rows focused.
...
A broad bounded query beats a narrow one returning 0.
...
- Use `get_series` for 1–2 known ids; `run_sql` for 3+ series, joins,
  aggregations.
- If a fetch comes back `"truncated": true`, aggregate in SQL
  (`GROUP BY date_trunc(...)`, a SUM/AVG/rank) rather than trying to pull the
  rest — a chart needs the aggregated result anyway.
